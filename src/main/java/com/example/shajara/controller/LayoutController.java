 ...
 * {
 *   n: [ { p:[id, parentId, stParentId, w, h], c:[childIds], v:[stChildIds], l, a, s, im, i, g, e, q, k, b } ],
 *   r: [rootId1, rootId2, ...],
 *   c: { "base": [orientation, levelSep, mixedSep, subtreeSep, siblingSep, layout, cols, collapse, assistantSep, partnerSep] },
 *   v: "version"
 * }
 *
...
public class LayoutController {
    ...
    private List<NodeInput> parseNodes(Map<String, Object> body) {
        ...
        Object nRaw = body.get("n");
        if (!(nRaw instanceof List)) return result;
        ...
        for (Object item : nList) {
            ...
            if ("null".equals(ni.parentId))   ni.parentId   = null;
            if ("null".equals(ni.stParentId)) ni.stParentId = null;

            // c = children IDs
            List<Object> c = (List<Object>) nodeMap.get("c");
            if (c != null) {
                for (Object cid : c) ni.childrenIds.add(String.valueOf(cid));
            }

            // v = stChildren IDs
            List<Object> v = (List<Object>) nodeMap.get("v");
            if (v != null) {
                for (Object vid : v) ni.stChildrenIds.add(String.valueOf(vid));
            }
    ...
    private List<String> parseRoots(Map<String, Object> body) {
        ...
        if (!(rRaw instanceof List)) return result;
    ...
    private Map<String, LayoutConfig> parseConfigs(Map<String, Object> body) {
        ...
        if (!(cRaw instanceof Map)) return result;
        ...
        for (Map.Entry<String, Object> entry : cMap.entrySet()) {
            ...
            if (val instanceof List) {
                // Array format: [orientation, levelSep, mixedSep, subtreeSep, siblingSep, layout, cols, collapse, assistantSep, partnerSep]
                List<Object> arr = (List<Object>) val;
                if (arr.size() > 0) lc.orientation                   = toInt(arr.get(0), 0);
                if (arr.size() > 1) lc.levelSeparation                = toDouble(arr.get(1), 60.0);
                if (arr.size() > 2) lc.mixedHierarchyNodesSeparation  = toDouble(arr.get(2), 15.0);
                if (arr.size() > 3) lc.subtreeSeparation              = toDouble(arr.get(3), 40.0);
                if (arr.size() > 4) lc.siblingSeparation              = toDouble(arr.get(4), 20.0);
                if (arr.size() > 5) lc.layout                         = toInt(arr.get(5), 0);
                if (arr.size() > 6) lc.columns                        = toInt(arr.get(6), 10);
                // arr[7] = collapse (skip)
                if (arr.size() > 8) lc.assistantSeparation            = toDouble(arr.get(8), 100.0);
                if (arr.size() > 9) lc.partnerNodeSeparation          = toDouble(arr.get(9), 15.0);

            } else if (val instanceof Map) {
                // Object format (legacy)
                Map<String, Object> m = (Map<String, Object>) val;
                if (m.containsKey("orientation"))       lc.orientation                  = toInt(m.get("orientation"), 0);
                if (m.containsKey("levelSeparation"))   lc.levelSeparation              = toDouble(m.get("levelSeparation"), 60.0);
                if (m.containsKey("subtreeSeparation")) lc.subtreeSeparation            = toDouble(m.get("subtreeSeparation"), 40.0);
                if (m.containsKey("siblingSeparation")) lc.siblingSeparation            = toDouble(m.get("siblingSeparation"), 20.0);
                if (m.containsKey("assistantSeparation")) lc.assistantSeparation        = toDouble(m.get("assistantSeparation"), 100.0);
                if (m.containsKey("partnerNodeSeparation")) lc.partnerNodeSeparation    = toDouble(m.get("partnerNodeSeparation"), 15.0);
            }
