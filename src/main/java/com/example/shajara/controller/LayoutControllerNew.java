...
import com.example.shajara.dto.layout1new.OrgChartLayoutCalculatorNew;
import com.example.shajara.dto.layout1new.OrgChartLayoutCalculatorNew.NodeInput;
import com.example.shajara.dto.layout1new.OrgChartLayoutCalculatorNew.LayoutConfig;
import com.example.shajara.dto.layout1new.OrgChartLayoutCalculatorNew.NodePosition;
 ...
 */
@RestController
@RequestMapping("/api/layout1new")
@RequiredArgsConstructor
public class LayoutControllerNew {

    private static final Logger log = LoggerFactory.getLogger(LayoutControllerNew.class);
    private static final ObjectMapper mapper = new ObjectMapper();

    private final OrgChartLayoutCalculatorNew calculator;
    ...
    public Map<String, Object> calculate(@RequestBody Map<String, Object> body) {
        ...
        try {
            ...
            log.info("=== /api/layout1new/calculate REQUEST ===");
        ...
        log.info("=== /api/layout1new/calculate DONE, returning {} nodes ===", response.size());
    ...
    private List<NodeInput> parseNodes(Map<String, Object> body) {
        ...
        for (Object item : nList) {
            ...
            if (nodeMap.containsKey("c")) {
                List<Object> cl = (List<Object>) nodeMap.get("c");
                for (Object cIdObj : cl) {
                    if (cIdObj != null) {
                        String cIdStr = String.valueOf(cIdObj);
                        if (!ni.childrenIds.contains(cIdStr)) {
                            ni.childrenIds.add(cIdStr);
                        }
                    }
                }
            }
            ...
            // stChildren = stc
            if (nodeMap.containsKey("stc")) {
                List<Object> stcl = (List<Object>) nodeMap.get("stc");
                for (Object stcIdObj : stcl) {
                    if (stcIdObj != null) {
                        String cIdStr = String.valueOf(stcIdObj);
                        if (!ni.stChildrenIds.contains(cIdStr)) {
                            ni.stChildrenIds.add(cIdStr);
                        }
                    }
                }
            } // l = layout type
