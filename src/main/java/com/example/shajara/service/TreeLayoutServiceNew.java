...
/**
 * Shajara layout algoritmi (V6 вЂ” adaptatsiya).
 *
 * Bu loyihada ma'lumotlar tuzilmasi:
 * - Person.fatherId / Person.motherId в†’ ota-ona
 * - Relation (type=SPOUSE) в†’ turmush o'rtoqlari
 * - findAllByFatherIdOrMotherId() в†’ bolalar
 *
 * Layout:
 * Y = -V_SPACE в†’ [ONA] [OTA]
 * Y = 0 в†’ [SP-L] ... [CENTER] ... [SP-R] (vertikal ustunlarda)
 * Y = +V_SPACE в†’ [bola1] [bola2] [bola3] ...
 */
@Service
...
public class TreeLayoutServiceNew {
    ...
    public TreeLayoutResponseDtoNew calculateLayout(Long personId) {
        ...
        // 3. TURMUSH O'RTOQLARI вЂ” ikki tomonda vertikal ustun
        ...
        for (TreeNodeDtoNew n : nodeMap.values()) {
    ...
    // =========================================================
    // BUTUN DARAXTNI QAYTARISH вЂ” Barcha persons
    ...
    // =========================================================
    // TURMUSH O'RTOQLARI вЂ” markaziy shaxs bilan bir qatorda (Y=0)
    // =========================================================
    private void placeSpouses(Person center,
        ...
        for (int i = 0; i < spouses.size(); i++) {
            ...
            if (i % 2 == 0) { // O'ng tomon
                x = RIGHT_COL_X;
                y = rightIdx * (NODE_H + CHILD_GAP);
                rightIdx++;
            } else { // Chap tomon
                x = LEFT_COL_X;
                y = leftIdx * (NODE_H + CHILD_GAP);
                leftIdx++;
            }
            ...
            double parentY = y - V_SPACE;
            double fatherX = (i % 2 == 0) ? x + (NODE_W / 2.0) : x - (NODE_W / 2.0);
            double motherX = (i % 2 == 0) ? x + (NODE_W * 1.5) + H_GAP : x - (NODE_W * 1.5) - H_GAP;

            if (spouse.getFatherId() != null) {
                personRepository.findById(spouse.getFatherId()).ifPresent(father -> {
                    if (!visited.contains(father.getId())) {
                        addNode(nodeMap, visited, father, fatherX, parentY, "FATHER");
            ...
            if (spouse.getMotherId() != null) {
                personRepository.findById(spouse.getMotherId()).ifPresent(mother -> {
                    if (!visited.contains(mother.getId())) {
                        addNode(nodeMap, visited, mother, motherX, parentY, "MOTHER");
    ...
    // =========================================================
    // AKA-UKALAR вЂ” markaziy shaxs bilan bir qatorda (chaproqda, Y=0)
    // =========================================================
    private void placeSiblings(Person center,
        ...
        List<Person> centerSpouses = relationRepository.findAllSpousesNative(center.getId());
        if (centerSpouses != null && centerSpouses.size() > 1) {
            // Because left spouses exist if there is more than 1 spouse overall, we shift base left by 1 column
            sibXBase -= (NODE_W + H_GAP);
        }
        ...
        for (Person s : siblings) {
            ...
            for (Person sc : siblingChildren) {
                if (!visited.contains(sc.getId())) {
                    ...
                    TreeNodeDtoNew targetNode = nodeMap.get(placementTargetId);
    ...
    // =========================================================
    // BOLALAR вЂ” rekursiv, ixtiyoriy chuqurlik
    ...
    private void placeChildren(Person center,
            ...
            Set<Long> visited,
            double childY) {
        // Rekursiv traversal вЂ” childY muhim emas (FamilyTree.js layout qiladi)
    ...
    private void addDescendantsRecursive(Person parent,
        ...
        TreeNodeDtoNew parentNode = nodeMap.get(parent.getId());
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            // в”Ђв”Ђ Farzandning juft(lar)ini qo'shish в”Ђв”Ђ
            ...
            for (int j = 0; j < childSpouses.size(); j++) {
                ...
                if (visited.contains(csp.getId())) {
                    // Allaqachon qo'shilgan в†’ faqat connection
            ...
            // в”Ђв”Ђ Rekursiv: farzandning avlodlari в”Ђв”Ђ
    ...
    // =========================================================
    private void resolveOverlaps(Map<Long, TreeNodeDtoNew> nodeMap) {
        Map<Double, List<TreeNodeDtoNew>> byRow = new LinkedHashMap<>();
        for (TreeNodeDtoNew n : nodeMap.values()) {
            byRow.computeIfAbsent(n.getY(), k -> new ArrayList<>()).add(n);
        }
        for (List<TreeNodeDtoNew> row : byRow.values()) {
            row.sort(Comparator.comparingDouble(TreeNodeDtoNew::getX));
            for (int i = 1; i < row.size(); i++) {
                TreeNodeDtoNew prev = row.get(i - 1);
                TreeNodeDtoNew curr = row.get(i);
    ...
    private void addNode(Map<Long, TreeNodeDtoNew> nodeMap, Set<Long> visited,
        ...
        nodeMap.put(p.getId(), TreeNodeDtoNew.builder()
                ...
                .photoUrl(p.getPhotoUrl())
                .treeId(p.getFamilyTree() != null ? p.getFamilyTree().getId() : null)
                .fatherId(p.getFatherId()) // вњ… to'g'ridan DB dan
                .motherId(p.getMotherId()) // вњ… to'g'ridan DB dan
                .x(x)
                .y(y)
                .build());
        visited.add(p.getId());
